...
class Application(tk.Tk):
    ...
    def initialize(self):
        ...
        filtro5_button = tk.Button(
            ...
            font=self.customFont)
        filtro6_button = tk.Button(
            buttosFrame,
            text='Invertir',
            command=lambda: self.buttonClicked('invertir'),
            font=self.customFont)
        filtro7_button = tk.Button(
            buttosFrame,
            text='Deshacer',
            command=lambda: self.buttonClicked('deshacer'),
            font=self.customFont)

        
        open_button = tk.Button(
            buttosFrame, text="Abrir", command=self.cargar_imagen, width=10, font=self.customFont)

        filtro1_button.grid(column=0, row=0,  sticky='news')
        filtro2_button.grid(column=1, row=0,  sticky='news')
        filtro3_button.grid(column=3, row=0,  sticky='news')
        filtro4_button.grid(column=4, row=0,  sticky='news')
        open_button.grid(column=7, row=0,  sticky='news')
        filtro5_button.grid(column=9, row=0,  sticky='news')
        filtro6_button.grid(column=2, row=0,  sticky='news')
        filtro7_button.grid(column=5, row=0,  sticky='news')
